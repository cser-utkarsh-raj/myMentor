...
def _supabase_jwks(issuer: str):
    ...
    if not (host.endswith(".supabase.co") or host == "supabase.co"):
...
def get_current_user(request: Request, credentials: Optional[HTTPAuthorizationCredentials] = Depends(security)) -> dict:
    ...
    # Demo mode is an explicit development path, never a fallback for bad credentials.
    if token == "demo_mode_token":
        return {"id": "demo-user-uuid", "email": "demo@mymentor.app"}

    if not token or token in ("local-demo-token", "null", "undefined"):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Authentication required")

    # Preferred: verify against the Supabase signing secret when configured.
    secrets = [settings.SUPABASE_JWT_SECRET, settings.JWT_SECRET]
    for secret in secrets:
        ...
        try:
            ...
            if user_id:
                return {"id": str(user_id), "email": str(payload.get("email") or "user@mymentor.app")}
        except JWTError:
            pass

    # Supabase projects using asymmetric signing keys (JWKS).
    try:
